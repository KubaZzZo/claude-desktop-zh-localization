...
`;
  ...
  const VISIBLE_TEXT_FIXES = new Map([
    ...
    ["What do you do for work?", "你的工作性质是？"],
    ["Status", "状态"],
    ["Project", "项目"],
    ["Environment", "环境"],
    ["Last activity", "最近活动"],
    ...
    ["Learn more", "了解更多"],
    ["Edit", "编辑"],
    ["Connectors have moved to", "连接器已迁移到"],
    ["Customize.", "自定义。"],
    ["Generate code, documents, and designs in a dedicated window alongside your conversation.", "在对话旁的独立窗口中生成代码、文档和设计。"],
    ["Skills have moved to", "技能已迁移到"],
    ["Instructions for Claude", "给 Claude 的指令"],
    ["Claude will keep these in mind across chats and Cowork within Anthropic's guidelines. Learn more", "只要符合 Anthropic 的使用准则，Claude 会在聊天和 Cowork 中记住这些内容。了解更多"],
    ["Claude will keeps these in mind across chats as long as they're within Anthropic's guidelines. These won't apply to Cowork or Code. Learn more", "只要符合 Anthropic 的使用准则，Claude 就会在各个聊天中记住这些内容。这些设置不适用于 Cowork 或 Code。了解更多"],
    ["You're running Claude through your organization's own inference provider", "你正在通过组织自己的推理服务提供商运行 Claude"],
    ["Your conversations are sent there, not to Anthropic, and are governed by your organization's agreement with that provider.", "你的对话会发送到该提供商，而不是 Anthropic，并受你的组织与该提供商协议约束。"],
    ...
    ["Your identity or account details", "你的身份或账户详细信息"],
    ["What Anthropic may receive (configured by your organization)", "Anthropic 可能接收的内容（由你的组织配置）"],
    ["Crash reports and error diagnostics, so we can fix bugs", "崩溃报告和错误诊断，以便我们修复问题"],
    ["Anonymous usage metrics including usage counts (not conversation content)", "匿名使用指标，包括使用次数（不包含对话内容）"],
    ["Update-check requests, so the app can stay current", "更新检查请求，以便应用保持最新"],
    ["A diagnostic report, only if you explicitly choose", "诊断报告，仅在你明确选择时发送"],
    ["Send to Anthropic", "发送给 Anthropic"],
    ["Preferences", "偏好设置"],
    ["Notifications", "通知"],
    ["Get notified when Claude has finished a response. Useful for long-running tasks.", "在 Claude 完成回复时通知你，适用于耗时较长的任务。"],
    ["Suitable for dyslexia", "适合阅读困难者"],
    ["Skills", "技能"],
  ]);

  const PHRASE_TEXT_FIXES = [
    [/^Code$/g, "代码"],
    [/^Cowork$/g, "协作"],
    [/^New task$/g, "新建任务"],
    [/^New session$/g, "新建会话"],
    ...
    [/^Name$/g, "名称"],
    [/^Full name$/g, "全名"],
    [/^What should Claude call you[?]$/g, "Claude 应该怎么称呼你？"],
    [/^What do you do for work[?]$/g, "你的工作性质是？"],
    [/^Status$/g, "状态"],
    [/^Project$/g, "项目"],
    [/^Environment$/g, "环境"],
    [/^Last activity$/g, "最近活动"],
    [/^Group by$/g, "分组方式"],
    [/^Sort by$/g, "排序方式"],
    [/^Active$/g, "活跃"],
    [/^Archived$/g, "已归档"],
    ...
    [/^Learn more$/g, "了解更多"],
    [/^Edit$/g, "编辑"],
    [/^Skills have moved to$/g, "技能已迁移到"],
    [/^Connectors have moved to$/g, "连接器已迁移到"],
    [/^Instructions for Claude$/g, "给 Claude 的指令"],
    [/^Preferences$/g, "偏好设置"],
    [/^Notifications$/g, "通知"],
    [/^Suitable for dyslexia$/g, "适合阅读困难者"],
    [/^Skills$/g, "技能"],
  ];

  function shouldFixTextNode(node) {{
    const parent = node.parentElement;
    if (!parent || parent.closest("script,style,[contenteditable='true'],pre,code")) return false;
    const scope = parent.closest("[role='dialog'],[role='menu'],[role='listbox'],main,section,aside,nav,header,footer");
    if (!scope) return false;
    const context = scope.innerText || "";
    return /(Appearance|外观|颜色模式|Color mode|聊天字体|Chat font|Font|字体|Theme|Match system|System|Code|Cowork|New task|New session|Projects|Scheduled|Customize|More|Sign out|Avatar|Full name|What should Claude call you|What do you do for work|新任务|项目|计划任务|自定义|Status|Project|Environment|Last activity|Group by|Sort by|Active|Archived|All|None|Recency|Date|State|Older|Ungrouped|Pinned|Drag to pin|Clear filters|All projects|Recents|Today|Yesterday|1d|3d|7d|14d|30d|Artifacts|Skills|Instructions for Claude|Preferences|Notifications|Suitable for dyslexia|Generate code, documents, and designs|Skills have moved to|Connectors have moved to|What Anthropic|Anthropic may receive|Anthropic doesn't see|Inference provider|Anthropic Serif|Anthropic Sans)/.test(context);
  }}
  ...
  function fixVisibleText(root = document.body) {{
    ...
    nodes.forEach((node) => {{
      ...
      if (replacement) next = next.replace(trimmed, replacement);
      PHRASE_TEXT_FIXES.forEach(([pattern, value]) => {{
...
def chunk_glob(pattern: str) -> str:
...
def main() -> int:
    ...
    for pattern, replacements in PATCHES.items():
        files = sorted(assets_dir.glob(chunk_glob(pattern)))
        for fpath in files:
